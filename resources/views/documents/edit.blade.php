...
<head>
    ...
    <title>Edit Dokumen - AsistenTA</title>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Edit Dokumen</h1>
                        <p class="text-gray-600">Perbarui informasi dan file dokumen</p>
                    </div>
                ...
                <form action="{{ route('documents.update', $document) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            placeholder="Contoh: Proposal Skripsi Bab 1-3" value="{{ old('title', $document->title) }}"
                            required>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            placeholder="Jelaskan tentang dokumen ini...">{{ old('description', $document->description) }}</textarea>
                    </div>
                        ...
                        <select name="meeting_id" id="meeting_id"
                            ...
                            @foreach ($meetings as $meeting)
                                <option value="{{ $meeting->id }}"
                                    {{ old('meeting_id', $document->meeting_id) == $meeting->id ? 'selected' : '' }}>
                                    {{ $meeting->title }} -
                    ...
                    <div class="mb-6">
                        ...
                        <!-- Pastikan select ini punya name="category" -->
                        <select name="category" id="category" required>
                            <option value="">-- Pilih Kategori --</option>
                            <option value="proposal"
                                {{ old('category', $document->category) == 'proposal' ? 'selected' : '' }}>Proposal
                            </option>
                            <option value="draft"
                                {{ old('category', $document->category) == 'draft' ? 'selected' : '' }}>Draft</option>
                            <option value="revisi"
                                {{ old('category', $document->category) == 'revisi' ? 'selected' : '' }}>Revisi
                            </option>
                            <option value="laporan"
                                {{ old('category', $document->category) == 'laporan' ? 'selected' : '' }}>Laporan
                            </option>
                            <option value="presentasi"
                                {{ old('category', $document->category) == 'presentasi' ? 'selected' : '' }}>Presentasi
                            </option>
                            <option value="final"
                                {{ old('category', $document->category) == 'final' ? 'selected' : '' }}>Final</option>
                            <option value="lainnya"
                                {{ old('category', $document->category) == 'lainnya' ? 'selected' : '' }}>Lainnya
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div
                            ...
                            <div id="fileUploadArea" class="cursor-pointer">
                                ...
                                <p class="text-lg font-medium text-gray-600">Klik untuk memilih file baru</p>
                                <p class="text-sm text-gray-500 mt-1">Kosongkan jika tidak mengganti file</p>
                        ...
                        @if ($document->file_path)
                            <div class="mt-3 p-3 bg-gray-50 rounded border border-gray-200">
                                ...
                                <div class="flex items-center mt-1">
                                    ...
                                    <span class="text-gray-800">{{ basename($document->file_path) }}</span>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                        ...
                        <ul class="text-sm text-blue-700 space-y-1">
                            <li>• Upload file baru hanya jika ingin mengganti file yang ada</li>
                            <li>• File maksimal 10MB</li>
                            <li>• Format yang didukung: PDF, DOC, DOCX, PPT, XLS, TXT, ZIP</li>
                            <li>• Status dokumen tetap tidak berubah setelah edit</li>
                        </ul>
                    </div>
