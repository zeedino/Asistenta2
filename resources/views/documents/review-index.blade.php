...
<head>
    ...
    <title>Review Dokumen - AsistenTA</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-7xl mx-auto">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">Review Dokumen Mahasiswa</h1>
                    <p class="text-gray-600">Review dan berikan feedback untuk dokumen mahasiswa bimbingan</p>
                </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-gray-800">{{ $submittedDocuments->count() }}</h3>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-gray-800">
                                {{ $reviewedDocuments->where('status', 'approved')->count() }}</h3>
                        </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-gray-800">
                                {{ $reviewedDocuments->where('status', 'rejected')->count() }}</h3>
                        </div>
                ...
                <div class="border-b border-gray-200">
                    <nav class="flex -mb-px">
                        <button id="submittedTab"
                            ...
                            Menunggu Review ({{ $submittedDocuments->count() }})
                        </button>
                        <button id="reviewedTab"
                            ...
                            Riwayat Review ({{ $reviewedDocuments->count() }})
                        </button>
                    </nav>
                </div>
            </div>
            ...
            <div id="submittedContent" class="tab-content">
                @if ($submittedDocuments->count() > 0)
                    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h2 class="text-xl font-semibold text-gray-800">Dokumen Menunggu Review</h2>
                        </div>
                ...
                @else
                    <div class="bg-white rounded-lg border border-gray-200 p-12 text-center">
                        ...
                        <h3 class="text-2xl font-bold text-gray-600 mb-2">Tidak Ada Dokumen Menunggu</h3>
                        <p class="text-gray-500 mb-6">Semua dokumen mahasiswa sudah direview.</p>
                    </div>
                @endif
            </div>

            <div id="reviewedContent" class="tab-content hidden">
                @if ($reviewedDocuments->count() > 0)
                    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h2 class="text-xl font-semibold text-gray-800">Riwayat Review</h2>
                        </div>
                        <div class="divide-y divide-gray-200">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex-1">
                                            <div class="flex items-start space-x-4">
                                                <div class="flex-shrink-0">
                                                    ...
                                                    {{ $document->status == 'approved' ? 'bg-green-100' : 'bg-red-100' }}">
                                                        <i
                                                            class="fas {{ $document->status == 'approved' ? 'fa-check-circle text-green-600' : 'fa-times-circle text-red-600' }} text-lg"></i>
                                                    </div>
                                                </div>
                                                <div class="flex-1">
                                                    <h3 class="font-semibold text-gray-800 text-lg">
                                                    ...
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                                                        ...
                                                        <div class="flex items-center text-sm text-gray-600">
                                                            ...
                                                            @if ($document->reviewed_at)
                                                                {{ $document->reviewed_at->format('d M Y H:i') }}
                                                            @else
                                                                Belum direview
                                                            @endif
                                                        </div>
                                                    ...
                                                    @if ($document->dosen_feedback)
                                                        <p class="text-sm text-gray-600 mt-2">
                                                            ...
                                                            {{ Str::limit($document->dosen_feedback, 120) }}
                                                        </p>
                                                    @endif
                                                    <div
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            {{ $document->status == 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ $document->status == 'approved' ? 'Disetujui' : 'Ditolak' }}
                                            </span>
                                            <a href="{{ route('documents.show', $document) }}"
                ...
                @else
                    <div class="bg-white rounded-lg border border-gray-200 p-12 text-center">
                        ...
                        <h3 class="text-2xl font-bold text-gray-600 mb-2">Belum Ada Riwayat Review</h3>
                        <p class="text-gray-500">Riwayat review akan muncul di sini.</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <script>
        ...
        document.getElementById('reviewedTab').addEventListener('click', function() {
            showTab('reviewed');
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(() => {
                // PERBAIKAN 3: Selector spesifik '.flash-message'
